...
unsigned char *base64_decode(const char *data, size_t input_length, size_t *output_length)
{
    ...
    *output_length = input_length / 4 * 3;
    ...
    for (size_t i = 0, j = 0; i < input_length;)
    {
        uint32_t sextet_a = data[i] == '=' ? 0 : b64_reverse_table[(unsigned char)data[i]];
        i++;
        uint32_t sextet_b = data[i] == '=' ? 0 : b64_reverse_table[(unsigned char)data[i]];
        i++;
        uint32_t sextet_c = data[i] == '=' ? 0 : b64_reverse_table[(unsigned char)data[i]];
        i++;
        uint32_t sextet_d = data[i] == '=' ? 0 : b64_reverse_table[(unsigned char)data[i]];
        i++;
...
static int is_valid_preset_name(const char *name)
{
    if (name[0] == '\0')
        return 0;
    if (strlen(name) >= MAX_PRESET_NAME)
        return 0;
...
static int load_profiles(void)
{
    ...
    snprintf(g_documentsPath, sizeof(g_documentsPath), "C:\\Users\\%s\\Documents\\My Games\\Rainbow Six - Siege\\", username);

    g_profileCount = 0;

#ifdef _WIN32
    char searchPath[600];
    snprintf(searchPath, sizeof(searchPath), "%s*", g_documentsPath);
    ...
    do
    {
        ...
        if (strcmp(findData.cFileName, ".") == 0 || strcmp(findData.cFileName, "..") == 0)
            continue;
        if (strcmp(findData.cFileName, "Benchmark") == 0)
            continue;
        if (g_profileCount >= MAX_PROFILES)
            break;

        strncpy(g_profiles[g_profileCount], findData.cFileName, sizeof(g_profiles[0]) - 1);
    ...
    while ((entry = readdir(d)) != NULL && g_profileCount < MAX_PROFILES)
    {
        if (strcmp(entry->d_name, ".") == 0 || strcmp(entry->d_name, "..") == 0)
            continue;
        if (strcmp(entry->d_name, "Benchmark") == 0)
            continue;
...
{
    char buf[32];
    if (json_arg(req, index, buf, sizeof(buf)) != 0)
        return default_val;
    return atoi(buf);
}
...
static void js_import_config(const char *seq, const char *req, void *arg)
{
    ...
    case 0:
        break;
    case 1:
        err = "Failed to decode config.";
        break;
    case 2:
        err = "Failed to decompress config.";
        break;
    case 3:
        err = "Decompressed size doesn't match expected payload size.";
        break;
    case 4:
        err = "Config doesn't contain header magic.";
        break;
    case 5:
        err = "Config version doesn't match app version.";
        break;
    case 6:
        err = "Config is corrupted.";
        break;
    case 7:
        err = "Invalid config.";
        break;
...
static void js_get_presets(const char *seq, const char *req, void *arg)
{
    ...
    for (int i = 0; i < g_presetCount; i++)
    {
        ...
        pos += (size_t)snprintf(json + pos, sizeof(json) - pos, "%s{\"name\":\"%s\",\"code\":\"%s\"}", i ? "," : "", nameEsc, codeEsc);
    }
    snprintf(json + pos, sizeof(json) - pos, "]");
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    webview_set_size(w, 430, 273, WEBVIEW_HINT_FIXED);
